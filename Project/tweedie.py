...
############ Data Preparation ############
cols=['Trip_Distance', 'Maintenance_flag','Engine_Load','Coolant_Temp', 'Intake_Pressure','Accel','Litres_Per_km','Throttle_Pos','Voltage',
'Ambient','Engine_Oil_Temp']
intCol=['Engine_Load','Coolant_Temp', 'Intake_Pressure','Accel','Litres_Per_km','Throttle_Pos','Voltage','Ambient','Engine_Oil_Temp']
cName=[]
inputData = pandas.read_csv('C:/Users/meenu/OneDrive/LinearNonLinearModels/Project/fleet_truck.csv',
                            delimiter = ',',usecols=cols)


inputData['Trip_Distance']=normalizeData(inputData['Trip_Distance'])
inputData=inputData.astype(float)
yName = 'Maintenance_flag'
eName = 'Trip_Distance'
trainData = inputData[inputData[eName] > 0.0].dropna().reset_index(drop=True)
y_train = trainData[yName]
e_train = trainData[eName]

# ############# Find the Tweedie P value and phi value ###################
xtab = pandas.pivot_table (trainData, index = intCol,
                           values = ['Maintenance_flag'], aggfunc = ['count', 'mean', 'var'])
...
Predictions['observed Maintenance_flag']=y_train
Predictions['predicted Maintenance_flag']=y_pred
Predictions['Adjusted Predicted Maintenance_flag'] = numpy.where(Predictions['predicted Maintenance_flag'] >= 0.4, 1, 0)
Predictions.to_csv("actual_pred.csv")
df_confusion = pandas.crosstab(Predictions['observed Maintenance_flag'], Predictions['Adjusted Predicted Maintenance_flag'])
print(df_confusion)
fScore=f1_score(Predictions['observed Maintenance_flag'], Predictions['Adjusted Predicted Maintenance_flag'], zero_division=1)
print(fScore)
# calculate ROC
ROC = roc_auc_score(Predictions['observed Maintenance_flag'], Predictions['Adjusted Predicted Maintenance_flag'])
print(ROC)
# # ############## Plotting Graph ####################
# plt.figure(figsize = (10,6), dpi = 200)
# plt.scatter(y_train,y_pred, marker = 'o', c = e_train, s = 20)
# plt.colorbar().set_label('Trip_Distance')
