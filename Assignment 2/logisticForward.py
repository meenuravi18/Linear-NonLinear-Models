...
HO_claim_history = pandas.read_csv('C:/Users/meenu/OneDrive/LinearNonLinearModels/Assignment 2/claim_history.csv', delimiter = ',')

HO_claim_history[['claim_indicator']] = numpy.where(HO_claim_history[['CLM_AMT']] > 0, 1, 0)

catlist =['MSTATUS', 'CAR_TYPE', 'REVOKED', 'URBANICITY']

trainData = HO_claim_history[['claim_indicator'] + catlist].dropna()
y = trainData['claim_indicator']

columnAT = pandas.get_dummies(trainData[['MSTATUS']].astype('category'))
columnM = pandas.get_dummies(trainData[['CAR_TYPE']].astype('category'))
columnPAT = pandas.get_dummies(trainData[['REVOKED']].astype('category'))
columnPG = pandas.get_dummies(trainData[['URBANICITY']].astype('category'))


table=pandas.DataFrame(index=[0,1,2,3,4])
table['parameters']=['Intercept','MSTATUS', 'CAR_TYPE', 'REVOKED', 'URBANICITY']
freeParam=[]
llh=[]
deviance=[]
dof=[]
